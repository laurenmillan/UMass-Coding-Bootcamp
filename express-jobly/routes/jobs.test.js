...
describe('DELETE /jobs/:id', function() {
	test('works for admin', async function() {
		const resp = await request(app).delete(`/jobs/${testJobIds[1]}`).set('authorization', `Bearer ${adminToken}`);
		expect(resp.body).toEqual({ deleted: testJobIds[1] });
	});

	test('unauth for others', async function() {
		const resp = await request(app).delete(`/jobs/${testJobIds[1]}`).set('authorization', `Bearer ${u1Token}`);
		expect(resp.statusCode).toEqual(401);
	});
